...
class Chat2SemanticReviewR194Tests(unittest.TestCase):
    def test_r194_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r194.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r194.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_CB1006D489AE13ABBE22")
        self.assertEqual(actual, expected)

    def test_r194_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r194.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "TradingPostListing": "CLIENT_CLASS_000965",
            },
        )

    def test_r194_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r194.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 194):
