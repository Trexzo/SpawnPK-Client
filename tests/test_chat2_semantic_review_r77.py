...
CLASS_COORDS = [
    ("CLIENT_CLASS_000133", "rs/d/s"),
    ("CLIENT_CLASS_000134", "rs/d/s$a"),
]
...
class Chat2SemanticReviewR77Tests(unittest.TestCase):
    def test_r77_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r77.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r77.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_BD18FEB5E201CDAFAD5D",
        )
        self.assertEqual(actual, expected)

    def test_r77_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r77.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ObjectSpawnOverrideManager": "CLIENT_CLASS_000133",
                "ObjectSpawnOverride": "CLIENT_CLASS_000134",
            },
        )

    def test_r77_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r77.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 77):
