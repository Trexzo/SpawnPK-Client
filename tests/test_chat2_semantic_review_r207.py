...
CLASS_COORDS = [
    ("CLIENT_CLASS_000476", "rs/l/f/a/j/a"),
    ("CLIENT_CLASS_000477", "rs/l/f/a/j/b"),
]
...
class Chat2SemanticReviewR207Tests(unittest.TestCase):
    def test_r207_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r207.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r207.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_AC7EFCB00B7B58248530")
        self.assertEqual(actual, expected)

    def test_r207_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r207.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "TooltipContent": "CLIENT_CLASS_000476",
                "TooltipOverlay": "CLIENT_CLASS_000477",
            },
        )

    def test_r207_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r207.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 207):
