...
CLASS_COORDS = [
    ("CLIENT_CLASS_000996", "rs/tools/TextPopupWindow$a"),
]
...
class Chat2SemanticReviewR200Tests(unittest.TestCase):
    def test_r200_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r200.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r200.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_F9DDD77407DE5A87FEFB")
        self.assertEqual(actual, expected)

    def test_r200_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r200.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "TextSearchWindow": "CLIENT_CLASS_000996",
            },
        )

    def test_r200_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r200.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 200):
