...
CLASS_COORDS = [
    ("CLIENT_CLASS_000987", "rs/t/a/e"),
    ("CLIENT_CLASS_000989", "rs/t/a/g"),
]
...
class Chat2SemanticReviewR81Tests(unittest.TestCase):
    def test_r81_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r81.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r81.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_B8D25FA5E0542C9A9585",
        )
        self.assertEqual(actual, expected)

    def test_r81_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r81.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "MapConfigLoader": "CLIENT_CLASS_000987",
                "WanderingMerchantItemConfigLoader": "CLIENT_CLASS_000989",
            },
        )

    def test_r81_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r81.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 81):
