...
def _class_lineage():
    return {
        ...
        "classes": [{
            "logical_id": "CLIENT_CLASS_000831",
            "semantic_name": None,
            "semantic_status": "UNKNOWN",
            "semantic_confidence": 0.0,
            "lineage": [{
                "build_id": "v308",
                "internal_name": "rs/runelite/a/o",
                "entry_path": "rs/runelite/a/o.class",
                "entry_sha256": "b" * 64,
                "structural_sha256": "c" * 64,
                "relation": "BASELINE",
                "confidence": 1.0,
                "provenance": [{"authority": "EXACT_CURRENT_CLIENT", "source": "chat2-r224-fixture"}],
            }],
            "semantic_provenance": [],
        }],
        "unresolved": [],
...
class Chat2SemanticReviewR224Tests(unittest.TestCase):
    def test_r224_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r224.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r224.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_833F07E8D4884EF03C77")
        self.assertEqual(actual, expected)

    def test_r224_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r224.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"Vertex": "CLIENT_CLASS_000831"},
        )

    def test_r224_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r224.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}
        prior_names = set()
        prior_owners = set()
        for batch in range(2, 224):
