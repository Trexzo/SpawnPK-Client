...
CLASS_COORDS = [
    ("CLIENT_CLASS_000428", "rs/l/e/g"),
    ("CLIENT_CLASS_000429", "rs/l/e/h"),
]
...
class Chat2SemanticReviewR182Tests(unittest.TestCase):
    def test_r182_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r182.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r182.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_73905BBB0E66D66FB778")
        self.assertEqual(actual, expected)

    def test_r182_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r182.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "HitDropEntry": "CLIENT_CLASS_000428",
                "HitDropManager": "CLIENT_CLASS_000429",
            },
        )

    def test_r182_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r182.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 182):
