...
CLASS_COORDS = [
    ("CLIENT_CLASS_001109", "rs/w/a"),
    ("CLIENT_CLASS_001110", "rs/w/b"),
]
...
class Chat2SemanticReviewR68Tests(unittest.TestCase):
    def test_r68_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r68.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r68.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_3C6B16405A7D17754AA5",
        )
        self.assertEqual(actual, expected)

    def test_r68_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r68.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "SoundEnvelope": "CLIENT_CLASS_001109",
                "SoundFilter": "CLIENT_CLASS_001110",
            },
        )

    def test_r68_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r68.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 68):
