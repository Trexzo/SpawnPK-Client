...
class Chat2SemanticReviewR53Tests(unittest.TestCase):
    def test_r53_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r53.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r53.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_E80B92D2470E8CC80E1D",
        )
        self.assertEqual(actual, expected)

    def test_r53_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r53.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ObjectManager": "CLIENT_CLASS_001114",
            },
        )

    def test_r53_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r53.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 53):
