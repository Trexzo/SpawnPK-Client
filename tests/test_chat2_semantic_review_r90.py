...
CLASS_COORDS = [
    ("CLIENT_CLASS_000344", "rs/k/n"),
    ("CLIENT_CLASS_000348", "rs/k/q"),
]
...
class Chat2SemanticReviewR90Tests(unittest.TestCase):
    def test_r90_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r90.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r90.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_68277D6628CE7C514BD0")
        self.assertEqual(actual, expected)

    def test_r90_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r90.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ShaderException": "CLIENT_CLASS_000344",
                "TextureManager": "CLIENT_CLASS_000348",
            },
        )

    def test_r90_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r90.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 90):
