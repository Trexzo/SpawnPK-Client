...
def _class_lineage():
    return {
        ...
        "classes": [
            {
                "logical_id": "CLIENT_CLASS_000328",
                "semantic_name": None,
                ...
                "lineage": [
                    {
                        "build_id": "v308",
                        "internal_name": "rs/k/c/b",
                        "entry_path": "rs/k/c/b.class",
                        "entry_sha256": "b" * 64,
...
class Chat2SemanticReviewR99Tests(unittest.TestCase):
    def test_r99_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r99.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r99.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_44B92BC956ED596303D4")
        self.assertEqual(actual, expected)

    def test_r99_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r99.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"GpuParticleMeshBuilder": "CLIENT_CLASS_000328"},
        )

    def test_r99_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r99.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 99):
