...
CLASS_COORDS = [
    ("CLIENT_CLASS_000281", "rs/gui/u"),
    ("CLIENT_CLASS_000282", "rs/gui/u$a"),
    ("CLIENT_CLASS_000283", "rs/gui/v"),
    ("CLIENT_CLASS_001096", "rs/ui/f"),
]
...
class Chat2SemanticReviewR147Tests(unittest.TestCase):
    def test_r147_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r147.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r147.json")
    ...
    def test_r147_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r147.json")
        self.assertEqual(
    ...
    def test_r147_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r147.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 147):
