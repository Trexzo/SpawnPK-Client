...
CLASS_COORDS = [
    ("CLIENT_CLASS_000451", "rs/l/f/a/c/b"),
]
...
class Chat2SemanticReviewR125Tests(unittest.TestCase):
    def test_r125_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r125.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r125.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_589B61E23E23DCA823B1")
        self.assertEqual(actual, expected)

    def test_r125_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r125.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "BossBarOverlayPacketHandler": "CLIENT_CLASS_000451",
            },
        )

    def test_r125_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r125.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 125):
