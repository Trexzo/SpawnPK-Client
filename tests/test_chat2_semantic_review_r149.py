...
CLASS_COORDS = [
    ("CLIENT_CLASS_001095", "rs/ui/e"),
    ("CLIENT_CLASS_001101", "rs/ui/k"),
]
...
class Chat2SemanticReviewR149Tests(unittest.TestCase):
    def test_r149_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r149.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r149.json")
    ...
    def test_r149_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r149.json")
        self.assertEqual(
    ...
    def test_r149_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r149.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 149):
