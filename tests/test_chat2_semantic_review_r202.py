...
CLASS_COORDS = [
    ("CLIENT_CLASS_000450", "rs/l/f/a/c/a"),
]
...
class Chat2SemanticReviewR202Tests(unittest.TestCase):
    def test_r202_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r202.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r202.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_614FACE224251DF0BFE0")
        self.assertEqual(actual, expected)

    def test_r202_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r202.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "BossBarOverlay": "CLIENT_CLASS_000450",
            },
        )

    def test_r202_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r202.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 202):
