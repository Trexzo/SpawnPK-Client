...
CLASS_COORDS = [
    ("CLIENT_CLASS_000168", "rs/f/a$a"),
    ("CLIENT_CLASS_000170", "rs/f/a$c"),
]
...
class Chat2SemanticReviewR84Tests(unittest.TestCase):
    def test_r84_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r84.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r84.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_190175A35620DE6C2C48",
        )
        self.assertEqual(actual, expected)

    def test_r84_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r84.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "GameVariant": "CLIENT_CLASS_000168",
                "ScreenMode": "CLIENT_CLASS_000170",
            },
        )

    def test_r84_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r84.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 84):
