...
CLASS_COORDS = [
    ("CLIENT_CLASS_000654", "rs/n/c/d/d"),
]
...
class Chat2SemanticReviewR123Tests(unittest.TestCase):
    def test_r123_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r123.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r123.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_9352C6F0A9D8EE075D3C")
        self.assertEqual(actual, expected)

    def test_r123_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r123.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "RaidInterfacePacketHandler": "CLIENT_CLASS_000654",
            },
        )

    def test_r123_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r123.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 123):
