...
class Chat2SemanticReviewR197Tests(unittest.TestCase):
    def test_r197_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r197.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r197.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_4C8C7F5FC8192D96005A")
        self.assertEqual(actual, expected)

    def test_r197_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r197.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ClientShortcutKeyHandler": "CLIENT_CLASS_000292",
            },
        )

    def test_r197_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r197.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 197):
