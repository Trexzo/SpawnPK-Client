...
class Chat2SemanticReviewR106Tests(unittest.TestCase):
    def test_r106_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r106.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r106.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_F7E321738CA3464384DD")
        self.assertEqual(actual, expected)

    def test_r106_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r106.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"ItemSpawnSearchResult": "CLIENT_CLASS_000853"},
        )

    def test_r106_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r106.json")
