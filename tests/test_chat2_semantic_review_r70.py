...
CLASS_COORDS = [
    ("CLIENT_CLASS_000099", "rs/cache/osrs/a"),
    ("CLIENT_CLASS_000100", "rs/cache/osrs/b"),
]
...
class Chat2SemanticReviewR70Tests(unittest.TestCase):
    def test_r70_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r70.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r70.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_E717D253AD955D38DC5D",
        )
        self.assertEqual(actual, expected)

    def test_r70_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r70.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "OsrsCacheIndex": "CLIENT_CLASS_000099",
                "OsrsAssetPacker": "CLIENT_CLASS_000100",
            },
        )

    def test_r70_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r70.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 70):
