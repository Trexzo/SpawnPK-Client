...
class Chat2SemanticReviewR104Tests(unittest.TestCase):
    def test_r104_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r104.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r104.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_7E5DE83AA771BC1BB53C")
        self.assertEqual(actual, expected)

    def test_r104_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r104.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"PerlinNoise": "CLIENT_CLASS_000739"},
        )

    def test_r104_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r104.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 104):
