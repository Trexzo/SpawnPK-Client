...
class Chat2SemanticReviewR87Tests(unittest.TestCase):
    def test_r87_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r87.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r87.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_2974461D01463567714E",
        )
        self.assertEqual(actual, expected)

    def test_r87_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r87.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "GpuRenderer": "CLIENT_CLASS_000333",
            },
        )

    def test_r87_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r87.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 87):
