...
CLASS_COORDS = [
    ("CLIENT_CLASS_000995", "rs/tools/TextPopupWindow"),
]
...
class Chat2SemanticReviewR198Tests(unittest.TestCase):
    def test_r198_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r198.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r198.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_527E4D7379140F46586A")
        self.assertEqual(actual, expected)

    def test_r198_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r198.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "TextPopupWindow": "CLIENT_CLASS_000995",
            },
        )

    def test_r198_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r198.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 198):
