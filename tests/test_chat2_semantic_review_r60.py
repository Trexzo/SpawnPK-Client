...
CLASS_COORDS = [
    ("CLIENT_CLASS_000143", "rs/e"),
    ("CLIENT_CLASS_000144", "rs/e$a"),
]
...
class Chat2SemanticReviewR60Tests(unittest.TestCase):
    def test_r60_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r60.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r60.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_F6010EA7F31DDA711385",
        )
        self.assertEqual(actual, expected)

    def test_r60_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r60.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "RegionClipDumper": "CLIENT_CLASS_000143",
                "RegionClipRecord": "CLIENT_CLASS_000144",
            },
        )

    def test_r60_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r60.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 60):
