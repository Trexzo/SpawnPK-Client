...
def _class_lineage():
    return {
        ...
        "classes": [{
            "logical_id":"CLIENT_CLASS_001065",
            "semantic_name":None,
            "semantic_status":"UNKNOWN",
            "semantic_confidence":0.0,
            "lineage":[{
                "build_id":"v308",
                "internal_name":"rs/ui/components/c",
                "entry_path":"rs/ui/components/c.class",
                "entry_sha256":"b"*64,
                "structural_sha256":"c"*64,
                "relation":"BASELINE",
                "confidence":1.0,
                "provenance":[{"authority":"EXACT_CURRENT_CLIENT","source":"chat2-r229-fixture"}]
            }],
            "semantic_provenance":[]
        }],
        "unresolved":[]
...
class Chat2SemanticReviewR229Tests(unittest.TestCase):
    def test_r229_resolves_deterministically(self):
        actual = resolve_semantic_candidates(
            _class_lineage(),
            _member_lineage(),
            _load("mappings/candidates/v308.semantic.chat2.r229.json"),
        )
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r229.json")
        self.assertEqual(actual["proposal_count"], 1)
        self.assertEqual(actual["unresolved"], [])
        self.assertEqual(actual["review_id"], "SEMREVIEW_204D3C4C23274DD2FC85")
        self.assertEqual(actual, expected)

    def test_r229_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r229.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"ComboBoxListRenderer":"CLIENT_CLASS_001065"},
        )

    def test_r229_name_and_owner_do_not_overlap_prior_reviews(self):
        current=_load("mappings/candidates/v308.semantic-review.chat2.r229.json")
