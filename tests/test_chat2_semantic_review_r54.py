...
CLASS_COORDS = [
    ("CLIENT_CLASS_000077", "rs/b"),
    ("CLIENT_CLASS_000295", "rs/i"),
]
...
class Chat2SemanticReviewR54Tests(unittest.TestCase):
    def test_r54_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r54.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r54.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_85873CA645C344480756",
        )
        self.assertEqual(actual, expected)

    def test_r54_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r54.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "RSCanvas": "CLIENT_CLASS_000077",
                "FileOperations": "CLIENT_CLASS_000295",
            },
        )

    def test_r54_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r54.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 54):
