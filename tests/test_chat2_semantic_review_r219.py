                ...
                "lineage": [
                    {
                        ...
                        "relation": "BASELINE",
                        "confidence": 1.0,
                        "provenance": [{"authority": "EXACT_CURRENT_CLIENT", "source": "chat2-r219-fixture"}],
                    }
                ],
                "semantic_provenance": [],
            }
...
class Chat2SemanticReviewR219Tests(unittest.TestCase):
    def test_r219_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r219.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r219.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_800BAAAFD12F32A54DC7")
        self.assertEqual(actual, expected)

    def test_r219_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r219.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"JagexColor": "CLIENT_CLASS_000440"},
        )

    def test_r219_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r219.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}
        prior_names = set()
        prior_owners = set()
        for batch in range(2, 219):
