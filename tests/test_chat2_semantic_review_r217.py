...
CLASS_COORDS = [
    ("CLIENT_CLASS_000424", "rs/l/e/d"),
    ("CLIENT_CLASS_000425", "rs/l/e/e"),
]
                ...
                "lineage": [
                    {
                        ...
                        "relation": "BASELINE",
                        "confidence": 1.0,
                        "provenance": [{"authority": "EXACT_CURRENT_CLIENT", "source": "chat2-r217-fixture"}],
                    }
                ],
                "semantic_provenance": [],
            }
...
class Chat2SemanticReviewR217Tests(unittest.TestCase):
    def test_r217_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r217.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r217.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_CBA61CC93BAF1D34A929")
        self.assertEqual(actual, expected)

    def test_r217_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r217.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ExperienceDropEntry": "CLIENT_CLASS_000424",
                "ExperienceDropManager": "CLIENT_CLASS_000425",
            },
        )

    def test_r217_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r217.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}
        prior_names = set()
        prior_owners = set()
        for batch in range(2, 217):
