...
CLASS_COORDS = [
    ("CLIENT_CLASS_000119", "rs/d/h$a"),
    ("CLIENT_CLASS_000132", "rs/d/r$a"),
]
...
class Chat2SemanticReviewR75Tests(unittest.TestCase):
    def test_r75_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r75.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r75.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_A2B10461C7E511AE5FFA",
        )
        self.assertEqual(actual, expected)

    def test_r75_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r75.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "FloorDefinitionMode": "CLIENT_CLASS_000119",
                "ObjectDefinitionMode": "CLIENT_CLASS_000132",
            },
        )

    def test_r75_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r75.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 75):
