...
CLASS_COORDS = [
    ("CLIENT_CLASS_000467", "rs/l/f/a/h/a"),
    ("CLIENT_CLASS_000468", "rs/l/f/a/h/b"),
]
...
class Chat2SemanticReviewR150Tests(unittest.TestCase):
    def test_r150_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r150.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r150.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_8C2F40C36F433BC19E4A")
        self.assertEqual(actual, expected)

    def test_r150_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r150.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "TextOverlay": "CLIENT_CLASS_000467",
                "TextOverlayPacketHandler": "CLIENT_CLASS_000468",
            },
        )

    def test_r150_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r150.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 150):
