...
def _class_lineage():
    return {
        ...
        "classes":[{
            "logical_id":"CLIENT_CLASS_001100",
            "semantic_name":None,
            "semantic_status":"UNKNOWN",
            "semantic_confidence":0.0,
            "lineage":[{
                "build_id":"v308",
                "internal_name":"rs/ui/j",
                "entry_path":"rs/ui/j.class",
                "entry_sha256":"b"*64,
                "structural_sha256":"c"*64,
                "relation":"BASELINE",
                "confidence":1.0,
                "provenance":[{"authority":"EXACT_CURRENT_CLIENT","source":"chat2-r232-fixture"}],
            }],
            "semantic_provenance":[],
        }],
        "unresolved":[],
...
class Chat2SemanticReviewR232Tests(unittest.TestCase):
    def test_r232_resolves_deterministically(self):
        actual=resolve_semantic_candidates(
            _class_lineage(),
            _member_lineage(),
            _load("mappings/candidates/v308.semantic.chat2.r232.json"),
        )
        expected=_load("mappings/candidates/v308.semantic-review.chat2.r232.json")
        self.assertEqual(actual["proposal_count"],1)
        self.assertEqual(actual["unresolved"],[])
        self.assertEqual(actual["review_id"],"SEMREVIEW_0B5B74A6D55CAADC04D6")
        self.assertEqual(actual,expected)

    def test_r232_expected_stable_id(self):
        review=_load("mappings/candidates/v308.semantic-review.chat2.r232.json")
        self.assertEqual(
            {row["proposed_name"]:row["stable_id"] for row in review["proposals"]},
            {"DynamicGridLayout":"CLIENT_CLASS_001100"},
        )

    def test_r232_name_and_owner_do_not_overlap_prior_reviews(self):
        current=_load("mappings/candidates/v308.semantic-review.chat2.r232.json")
