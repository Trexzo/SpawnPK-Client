...
CLASS_COORDS = [
    ("CLIENT_CLASS_000334", "rs/k/e$a"),
    ("CLIENT_CLASS_000339", "rs/k/j"),
]
...
class Chat2SemanticReviewR88Tests(unittest.TestCase):
    def test_r88_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r88.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r88.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_E36234AB09C85C8F04D8",
        )
        self.assertEqual(actual, expected)

    def test_r88_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r88.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ComputeMode": "CLIENT_CLASS_000334",
                "OSType": "CLIENT_CLASS_000339",
            },
        )

    def test_r88_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r88.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 88):
