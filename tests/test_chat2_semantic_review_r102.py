...
CLASS_COORDS = [
    ("CLIENT_CLASS_000101", "rs/cache/osrs/c"),
]
...
class Chat2SemanticReviewR102Tests(unittest.TestCase):
    def test_r102_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r102.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r102.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_BFB4BC3B81C92AA0DCA1")
        self.assertEqual(actual, expected)

    def test_r102_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r102.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"OsrsAssetState": "CLIENT_CLASS_000101"},
        )

    def test_r102_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r102.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 102):
