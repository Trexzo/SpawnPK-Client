...
CLASS_COORDS = [
    ("CLIENT_CLASS_000491", "rs/l/f/j"),
    ("CLIENT_CLASS_000494", "rs/l/f/m"),
]
                ...
                "lineage": [
                    {
                        ...
                        "relation": "BASELINE",
                        "confidence": 1.0,
                        "provenance": [{"authority": "EXACT_CURRENT_CLIENT", "source": "chat2-r215-fixture"}],
                    }
                ],
                "semantic_provenance": [],
            }
...
class Chat2SemanticReviewR215Tests(unittest.TestCase):
    def test_r215_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r215.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r215.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_48B9458A7F27AEBC8AD9")
        self.assertEqual(actual, expected)

    def test_r215_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r215.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "OverlayDragHotkeyListener": "CLIENT_CLASS_000491",
                "OverlayUtil": "CLIENT_CLASS_000494",
            },
        )

    def test_r215_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r215.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}
        prior_names = set()
        prior_owners = set()
        for batch in range(2, 215):
