...
CLASS_COORDS = [
    ("CLIENT_CLASS_000894", "rs/s/c/g"),
    ("CLIENT_CLASS_000895", "rs/s/c/h"),
]
...
class Chat2SemanticReviewR196Tests(unittest.TestCase):
    def test_r196_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r196.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r196.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_A24A3EDEADBED527A066")
        self.assertEqual(actual, expected)

    def test_r196_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r196.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "DeveloperToolsPluginPanel": "CLIENT_CLASS_000894",
                "DevToolsWidgetType": "CLIENT_CLASS_000895",
            },
        )

    def test_r196_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r196.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 196):
