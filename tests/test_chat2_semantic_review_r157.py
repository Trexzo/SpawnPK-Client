...
CLASS_COORDS = [
    ("CLIENT_CLASS_000394", "rs/l/d/c$a"),
]
...
class Chat2SemanticReviewR157Tests(unittest.TestCase):
    def test_r157_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r157.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r157.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_2FFAECB55B981427C7BA")
        self.assertEqual(actual, expected)

    def test_r157_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r157.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "LoginScreenState": "CLIENT_CLASS_000394",
            },
        )

    def test_r157_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r157.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 157):
