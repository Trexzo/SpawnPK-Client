...
CLASS_COORDS = [
    ("CLIENT_CLASS_000465", "rs/l/f/a/g/b"),
]
...
class Chat2SemanticReviewR206Tests(unittest.TestCase):
    def test_r206_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r206.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r206.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_2EB868EB5DFC9004D925")
        self.assertEqual(actual, expected)

    def test_r206_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r206.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "RaidProgressBar": "CLIENT_CLASS_000465",
            },
        )

    def test_r206_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r206.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 206):
