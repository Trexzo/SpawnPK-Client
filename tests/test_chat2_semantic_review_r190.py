...
CLASS_COORDS = [("CLIENT_CLASS_000959", "rs/s/r/b")]
    ...
    for logical_id, internal_name in CLASS_COORDS:
        classes.append({
            ...
            "lineage": [{
                ...
                "relation": "BASELINE",
                "confidence": 1.0,
                "provenance": [{"authority": "EXACT_CURRENT_CLIENT", "source": "chat2-r190-fixture"}],
            }],
            "semantic_provenance": [],
        })
    return {
...
def _member_lineage():
    return {
        "schema_version": 1, "kind": "member_lineage", "class_namespace": "spawnpk-client",
...
class Chat2SemanticReviewR190Tests(unittest.TestCase):
    def test_r190_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r190.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r190.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_6342E35B4E2884955FF4")
        self.assertEqual(actual, expected)

    def test_r190_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r190.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"TileIndicatorsOverlay": "CLIENT_CLASS_000959"},
        )

    def test_r190_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r190.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}
        prior_names = set()
        prior_owners = set()
        for batch in range(2, 190):
