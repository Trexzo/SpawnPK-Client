...
CLASS_COORDS = [
    ("CLIENT_CLASS_000138", "rs/d/w"),
    ("CLIENT_CLASS_000139", "rs/d/w$a"),
]
...
class Chat2SemanticReviewR73Tests(unittest.TestCase):
    def test_r73_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r73.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r73.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_33ABFA68EADCBAD63204",
        )
        self.assertEqual(actual, expected)

    def test_r73_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r73.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "BloodCurrencyValueConfig": "CLIENT_CLASS_000138",
                "BloodCurrencyType": "CLIENT_CLASS_000139",
            },
        )

    def test_r73_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r73.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 73):
