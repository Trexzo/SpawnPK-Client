...
CLASS_COORDS = [
    ("CLIENT_CLASS_000368", "rs/l/b/a/a"),
]
...
class Chat2SemanticReviewR110Tests(unittest.TestCase):
    def test_r110_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r110.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r110.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_DEABD93A8D21306DEBDA")
        self.assertEqual(actual, expected)

    def test_r110_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r110.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"EditorKitUtilityCommandHandler": "CLIENT_CLASS_000368"},
        )

    def test_r110_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r110.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 110):
