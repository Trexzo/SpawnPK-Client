...
CLASS_COORDS = [
    ("CLIENT_CLASS_000009", "rs/A/f"),
    ("CLIENT_CLASS_000018", "rs/A/o"),
]
...
class Chat2SemanticReviewR82Tests(unittest.TestCase):
    def test_r82_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r82.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r82.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_2C02DF701BB8B1FC8FAF",
        )
        self.assertEqual(actual, expected)

    def test_r82_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r82.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ColorGsonAdapter": "CLIENT_CLASS_000009",
                "MacOSWindowUtil": "CLIENT_CLASS_000018",
            },
        )

    def test_r82_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r82.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 82):
