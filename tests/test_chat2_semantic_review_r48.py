...
CLASS_COORDS = [
    ("CLIENT_CLASS_000028", "rs/C"),
    ("CLIENT_CLASS_000032", "rs/E"),
]
...
class Chat2SemanticReviewR48Tests(unittest.TestCase):
    def test_r48_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r48.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r48.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_48C0EF3660A50E61972B",
        )
        self.assertEqual(actual, expected)

    def test_r48_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r48.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "RSApplet": "CLIENT_CLASS_000028",
                "RSFrame": "CLIENT_CLASS_000032",
            },
        )

    def test_r48_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r48.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 48):
