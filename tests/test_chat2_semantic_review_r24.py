...
class Chat2SemanticReviewR24Tests(unittest.TestCase):
    def test_r24_resolves_deterministically(self):
        candidates = _load(
            "mappings/candidates/"
            "v308.semantic.chat2.r24.json"
        )
        expected = _load(
            "mappings/candidates/"
            "v308.semantic-review.chat2.r24.json"
        )
    ...
    def test_r24_expected_stable_ids(self):
        review = _load(
            "mappings/candidates/"
            "v308.semantic-review.chat2.r24.json"
        )
    ...
    def test_r24_names_do_not_overlap_prior_reviews(self):
        r24 = _load(
            "mappings/candidates/"
            "v308.semantic-review.chat2.r24.json"
        )
        r24_names = {
            row["proposed_name"]
            for row in r24["proposals"]
        }

        prior_names = set()
        for batch in range(2, 24):
            path = (
                ROOT
                / "mappings"
                / "candidates"
        ...
        self.assertTrue(r24_names.isdisjoint(prior_names))
