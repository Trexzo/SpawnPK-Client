...
class Chat2SemanticReviewR85Tests(unittest.TestCase):
    def test_r85_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r85.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r85.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_E90B9897A7720ADF272D",
        )
        self.assertEqual(actual, expected)

    def test_r85_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r85.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "DepthBufferedImageSurface": "CLIENT_CLASS_000352",
            },
        )

    def test_r85_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r85.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 85):
