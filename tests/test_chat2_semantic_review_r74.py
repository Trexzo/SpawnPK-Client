...
CLASS_COORDS = [
    ("CLIENT_CLASS_000125", "rs/d/n"),
    ("CLIENT_CLASS_000126", "rs/d/n$a"),
]
...
class Chat2SemanticReviewR74Tests(unittest.TestCase):
    def test_r74_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r74.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r74.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_1EDA94B7784633DD0C3D",
        )
        self.assertEqual(actual, expected)

    def test_r74_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r74.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ItemIconOverlay": "CLIENT_CLASS_000125",
                "ItemIconOverlayType": "CLIENT_CLASS_000126",
            },
        )

    def test_r74_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r74.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 74):
