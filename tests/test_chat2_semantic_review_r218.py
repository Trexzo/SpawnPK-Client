...
CLASS_COORDS = [
    ("CLIENT_CLASS_000431", "rs/l/e/j"),
    ("CLIENT_CLASS_000432", "rs/l/e/k"),
]
                ...
                "lineage": [
                    {
                        ...
                        "relation": "BASELINE",
                        "confidence": 1.0,
                        "provenance": [{"authority": "EXACT_CURRENT_CLIENT", "source": "chat2-r218-fixture"}],
                    }
                ],
                "semantic_provenance": [],
            }
...
class Chat2SemanticReviewR218Tests(unittest.TestCase):
    def test_r218_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r218.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r218.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_DB704E01C67994C03183")
        self.assertEqual(actual, expected)

    def test_r218_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r218.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "InteractiveModelHighlight": "CLIENT_CLASS_000431",
                "InteractiveModelHighlightRegistry": "CLIENT_CLASS_000432",
            },
        )

    def test_r218_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r218.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}
        prior_names = set()
        prior_owners = set()
        for batch in range(2, 218):
