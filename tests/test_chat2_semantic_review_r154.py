...
class Chat2SemanticReviewR154Tests(unittest.TestCase):
    def test_r154_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r154.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r154.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_26EFDAFE2D0D62B5BA26")
        self.assertEqual(actual, expected)

    def test_r154_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r154.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "SeasonalItemMode": "CLIENT_CLASS_000171",
            },
        )

    def test_r154_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r154.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 154):
