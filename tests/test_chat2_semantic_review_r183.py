...
CLASS_COORDS = [
    ("CLIENT_CLASS_000954", "rs/s/q/b/a"),
]
...
class Chat2SemanticReviewR183Tests(unittest.TestCase):
    def test_r183_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r183.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r183.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_A3664323BAFEE620DA1D")
        self.assertEqual(actual, expected)

    def test_r183_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r183.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"FightPerformancePanel": "CLIENT_CLASS_000954"},
        )

    def test_r183_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r183.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 183):
