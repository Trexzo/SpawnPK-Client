...
CLASS_COORDS = [
    ("CLIENT_CLASS_000193", "rs/gui/J"),
    ("CLIENT_CLASS_000260", "rs/gui/c/a"),
]
...
class Chat2SemanticReviewR145Tests(unittest.TestCase):
    def test_r145_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r145.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r145.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_CEECF5480BE96957E887")
        self.assertEqual(actual, expected)

    def test_r145_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r145.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ClientSidebarPanel": "CLIENT_CLASS_000193",
                "PvPTrackerSidebarPanel": "CLIENT_CLASS_000260",
            },
        )

    def test_r145_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r145.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 145):
