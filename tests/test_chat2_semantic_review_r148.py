...
CLASS_COORDS = [
    ("CLIENT_CLASS_001016", "rs/ui/a"),
    ("CLIENT_CLASS_001027", "rs/ui/b"),
    ("CLIENT_CLASS_001032", "rs/ui/c"),
    ("CLIENT_CLASS_001104", "rs/ui/m"),
]
...
class Chat2SemanticReviewR148Tests(unittest.TestCase):
    def test_r148_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r148.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r148.json")
    ...
    def test_r148_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r148.json")
        self.assertEqual(
    ...
    def test_r148_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r148.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 148):
