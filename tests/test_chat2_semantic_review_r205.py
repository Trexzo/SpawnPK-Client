...
CLASS_COORDS = [
    ("CLIENT_CLASS_000478", "rs/l/f/a/k/a"),
]
...
class Chat2SemanticReviewR205Tests(unittest.TestCase):
    def test_r205_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r205.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r205.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_3D483B02865CAEB929A5")
        self.assertEqual(actual, expected)

    def test_r205_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r205.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "AdventureBookChapterProgressRenderer": "CLIENT_CLASS_000478",
            },
        )

    def test_r205_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r205.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 205):
