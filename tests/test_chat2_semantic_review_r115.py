...
CLASS_COORDS = [
    ("CLIENT_CLASS_000745", "rs/q/a/a/a"),
    ("CLIENT_CLASS_000762", "rs/q/a/a/b"),
]
...
class Chat2SemanticReviewR115Tests(unittest.TestCase):
    def test_r115_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r115.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r115.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_6A2949D5AE0162BAD9B4")
        self.assertEqual(actual, expected)

    def test_r115_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r115.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ScriptPacketHandler": "CLIENT_CLASS_000745",
                "ScriptPacketDispatcher": "CLIENT_CLASS_000762",
            },
        )

    def test_r115_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r115.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 115):
