...
CLASS_COORDS = [
    ("CLIENT_CLASS_000951", "rs/s/q/a/a"),
    ("CLIENT_CLASS_000952", "rs/s/q/a/b"),
]
...
class Chat2SemanticReviewR142Tests(unittest.TestCase):
    def test_r142_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r142.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r142.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_777E2500491C487C174F")
        self.assertEqual(actual, expected)

    def test_r142_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r142.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "PvPTrackerFight": "CLIENT_CLASS_000951",
                "PvPTrackerFighterStats": "CLIENT_CLASS_000952",
            },
        )

    def test_r142_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r142.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 142):
