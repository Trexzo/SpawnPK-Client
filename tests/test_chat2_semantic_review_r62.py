...
CLASS_COORDS = [
    ("CLIENT_CLASS_001121", "rs/y"),
    ("CLIENT_CLASS_001125", "rs/z"),
]
...
class Chat2SemanticReviewR62Tests(unittest.TestCase):
    def test_r62_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r62.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r62.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_142F0F9C2756A947D982",
        )
        self.assertEqual(actual, expected)

    def test_r62_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r62.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "LegacySequenceConfigLoader": "CLIENT_CLASS_001121",
                "LegacySpotAnimationConfigLoader": "CLIENT_CLASS_001125",
            },
        )

    def test_r62_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r62.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 62):
