...
CLASS_COORDS = [
    ("CLIENT_CLASS_001112", "rs/w/d"),
    ("CLIENT_CLASS_001113", "rs/w/e"),
]
...
class Chat2SemanticReviewR69Tests(unittest.TestCase):
    def test_r69_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r69.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r69.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_6ED5FDFC81606A4F1D6C",
        )
        self.assertEqual(actual, expected)

    def test_r69_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r69.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "Instrument": "CLIENT_CLASS_001112",
                "SoundEffect": "CLIENT_CLASS_001113",
            },
        )

    def test_r69_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r69.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 69):
