...
CLASS_COORDS = [
    ("CLIENT_CLASS_000638", "rs/n/c/b/d"),
]
...
class Chat2SemanticReviewR126Tests(unittest.TestCase):
    def test_r126_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r126.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r126.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_18F45D97BD3C38C9A5C7")
        self.assertEqual(actual, expected)

    def test_r126_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r126.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "GladiatorsVindicationWorldEventInterfacePacketHandler": "CLIENT_CLASS_000638",
            },
        )

    def test_r126_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r126.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 126):
