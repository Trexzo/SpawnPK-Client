...
CLASS_COORDS = [
    ("CLIENT_CLASS_000315", "rs/k/a/a"),
    ("CLIENT_CLASS_000316", "rs/k/a/b"),
    ("CLIENT_CLASS_000317", "rs/k/a/c"),
]
...
class Chat2SemanticReviewR93Tests(unittest.TestCase):
    def test_r93_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r93.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r93.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_161E3E5F88A625FE06BD")
        self.assertEqual(actual, expected)

    def test_r93_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r93.json")
        self.assertEqual(
    ...
    def test_r93_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r93.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 93):
