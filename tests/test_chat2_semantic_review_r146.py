...
class Chat2SemanticReviewR146Tests(unittest.TestCase):
    def test_r146_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r146.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r146.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_08BBF1DCA1080D2650C2")
        self.assertEqual(actual, expected)

    def test_r146_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r146.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ItemSearchSidebarPanel": "CLIENT_CLASS_000190",
                "DeveloperToolsSidebarPanel": "CLIENT_CLASS_000890",
            },
        )

    def test_r146_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r146.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 146):
