...
CLASS_COORDS = [
    ("CLIENT_CLASS_000102", "rs/cache/osrs/util/OsrsMapDependencyScanner"),
]
...
class Chat2SemanticReviewR199Tests(unittest.TestCase):
    def test_r199_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r199.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r199.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_34B97388290790B025D2")
        self.assertEqual(actual, expected)

    def test_r199_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r199.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "OsrsMapDependencyScanner": "CLIENT_CLASS_000102",
            },
        )

    def test_r199_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r199.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 199):
