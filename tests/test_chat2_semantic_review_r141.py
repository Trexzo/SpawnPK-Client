...
CLASS_COORDS = [
    ("CLIENT_CLASS_000401", "rs/l/e/a/c"),
    ("CLIENT_CLASS_000402", "rs/l/e/a/d"),
]
...
class Chat2SemanticReviewR141Tests(unittest.TestCase):
    def test_r141_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r141.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r141.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_14F944AD06349D26BCA6")
        self.assertEqual(actual, expected)

    def test_r141_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r141.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "PvPTrackerOverlay": "CLIENT_CLASS_000401",
                "PvPTrackerPacketHandler": "CLIENT_CLASS_000402",
            },
        )

    def test_r141_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r141.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 141):
