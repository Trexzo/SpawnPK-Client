...
class Chat2SemanticReviewR174Tests(unittest.TestCase):
    def test_r174_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r174.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r174.json")
    ...
    def test_r174_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r174.json")
        self.assertEqual(
    ...
    def test_r174_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r174.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 174):
