...
CLASS_COORDS = [
    ("CLIENT_CLASS_000947", "rs/s/p/b"),
    ("CLIENT_CLASS_000948", "rs/s/p/c"),
]
...
class Chat2SemanticReviewR187Tests(unittest.TestCase):
    def test_r187_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r187.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r187.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_75AAAF2DB6589C9AAFAB")
        self.assertEqual(actual, expected)

    def test_r187_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r187.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "PlayerOutlineOverlay": "CLIENT_CLASS_000947",
                "PetOutlineOverlay": "CLIENT_CLASS_000948",
            },
        )

    def test_r187_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r187.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 187):
