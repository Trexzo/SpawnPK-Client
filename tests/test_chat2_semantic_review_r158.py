...
CLASS_COORDS = [
    ("CLIENT_CLASS_000391", "rs/l/d/a"),
    ("CLIENT_CLASS_000392", "rs/l/d/b"),
]
...
class Chat2SemanticReviewR158Tests(unittest.TestCase):
    def test_r158_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r158.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r158.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_5257431E8CC3A44230A3")
        self.assertEqual(actual, expected)

    def test_r158_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r158.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "LoginOrb": "CLIENT_CLASS_000391",
                "LoginMessageBox": "CLIENT_CLASS_000392",
            },
        )

    def test_r158_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r158.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 158):
