...
CLASS_COORDS = [
    ("CLIENT_CLASS_000833", "rs/runelite/b/a"),
    ("CLIENT_CLASS_000834", "rs/runelite/b/a$a"),
]
    ...
    for logical_id, internal_name in CLASS_COORDS:
        classes.append({
            ...
            "lineage": [{
                ...
                "relation": "BASELINE",
                "confidence": 1.0,
                "provenance": [{"authority": "EXACT_CURRENT_CLIENT", "source": "chat2-r223-fixture"}],
            }],
            "semantic_provenance": [],
        })
    return {
...
class Chat2SemanticReviewR223Tests(unittest.TestCase):
    def test_r223_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r223.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r223.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_5A82D8014F00A60B0954")
        self.assertEqual(actual, expected)

    def test_r223_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r223.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "RenderableDrawListenerRegistry": "CLIENT_CLASS_000833",
                "RenderableDrawListener": "CLIENT_CLASS_000834",
            },
        )

    def test_r223_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r223.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}
        prior_names = set()
        prior_owners = set()
        for batch in range(2, 223):
