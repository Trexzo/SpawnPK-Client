...
class Chat2SemanticReviewR192Tests(unittest.TestCase):
    def test_r192_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r192.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r192.json")
    ...
    def test_r192_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r192.json")
        self.assertEqual(
    ...
    def test_r192_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r192.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 192):
