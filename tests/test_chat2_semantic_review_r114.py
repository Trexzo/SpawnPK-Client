...
CLASS_COORDS = [
    ("CLIENT_CLASS_000419", "rs/l/e/a/u"),
]
...
class Chat2SemanticReviewR114Tests(unittest.TestCase):
    def test_r114_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r114.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r114.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_966D74C50FB581DF0BB0")
        self.assertEqual(actual, expected)

    def test_r114_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r114.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"TextureBrowserOverlay": "CLIENT_CLASS_000419"},
        )

    def test_r114_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r114.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 114):
