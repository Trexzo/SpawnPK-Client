...
CLASS_COORDS = [
    ("CLIENT_CLASS_000114", "rs/d/e"),
    ("CLIENT_CLASS_000115", "rs/d/e$a"),
]
...
class Chat2SemanticReviewR76Tests(unittest.TestCase):
    def test_r76_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r76.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r76.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_C5E41431CAE19B2F928A",
        )
        self.assertEqual(actual, expected)

    def test_r76_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r76.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "NpcObjectIconRenderer": "CLIENT_CLASS_000114",
                "NpcObjectIconType": "CLIENT_CLASS_000115",
            },
        )

    def test_r76_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r76.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 76):
