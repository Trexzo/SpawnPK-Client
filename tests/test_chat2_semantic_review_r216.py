...
CLASS_COORDS = [
    ("CLIENT_CLASS_000422", "rs/l/e/b"),
    ("CLIENT_CLASS_000423", "rs/l/e/c"),
]
                ...
                "lineage": [
                    {
                        ...
                        "relation": "BASELINE",
                        "confidence": 1.0,
                        "provenance": [{"authority": "EXACT_CURRENT_CLIENT", "source": "chat2-r216-fixture"}],
                    }
                ],
                "semantic_provenance": [],
            }
...
class Chat2SemanticReviewR216Tests(unittest.TestCase):
    def test_r216_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r216.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r216.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_70C98BE24E8894C7D70C")
        self.assertEqual(actual, expected)

    def test_r216_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r216.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "CommandConsoleEntry": "CLIENT_CLASS_000422",
                "CommandConsole": "CLIENT_CLASS_000423",
            },
        )

    def test_r216_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r216.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}
        prior_names = set()
        prior_owners = set()
        for batch in range(2, 216):
