...
class Chat2SemanticReviewR193Tests(unittest.TestCase):
    def test_r193_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r193.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r193.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_F96D5656110D072AF7CE")
        self.assertEqual(actual, expected)

    def test_r193_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r193.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "NotesSaveFocusListener": "CLIENT_CLASS_000933",
            },
        )

    def test_r193_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r193.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 193):
