...
CLASS_COORDS = [
    ...
    ("CLIENT_CLASS_000196", "rs/gui/Launcher"),
]
...
class Chat2SemanticReviewR170Tests(unittest.TestCase):
    def test_r170_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r170.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r170.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_FEA952C771DB52B91A41")
        self.assertEqual(actual, expected)

    def test_r170_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r170.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "Client": "CLIENT_CLASS_000029",
                "Launcher": "CLIENT_CLASS_000196",
            },
        )
        self.assertTrue(all(row["confidence"] == 1.0 for row in review["proposals"]))

    def test_r170_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r170.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 170):
