...
CLASS_COORDS = [
    ("CLIENT_CLASS_000039", "rs/L"),
    ("CLIENT_CLASS_000041", "rs/N"),
]
...
class Chat2SemanticReviewR49Tests(unittest.TestCase):
    def test_r49_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r49.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r49.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_CEA31CD5E3BE66DE0413",
        )
        self.assertEqual(actual, expected)

    def test_r49_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r49.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "AudioClipPlayer": "CLIENT_CLASS_000039",
                "SpawnObjectNode": "CLIENT_CLASS_000041",
            },
        )

    def test_r49_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r49.json")
