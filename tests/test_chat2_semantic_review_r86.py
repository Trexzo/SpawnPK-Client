...
class Chat2SemanticReviewR86Tests(unittest.TestCase):
    def test_r86_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r86.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r86.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_D1F558732D8E7A698664",
        )
        self.assertEqual(actual, expected)

    def test_r86_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r86.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "GpuRuntimeSettings": "CLIENT_CLASS_000318",
            },
        )

    def test_r86_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r86.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 86):
