...
CLASS_COORDS = [
    ("CLIENT_CLASS_000811", "rs/runelite/a/c"),
]
...
class Chat2SemanticReviewR159Tests(unittest.TestCase):
    def test_r159_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r159.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r159.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_3BB4E2A255F6BCA26229")
        self.assertEqual(actual, expected)

    def test_r159_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r159.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "GameState": "CLIENT_CLASS_000811",
            },
        )

    def test_r159_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r159.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 159):
