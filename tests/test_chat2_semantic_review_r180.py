...
class Chat2SemanticReviewR180Tests(unittest.TestCase):
    def test_r180_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r180.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r180.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_DA004CEB888CF9F43859")
        self.assertEqual(actual, expected)

    def test_r180_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r180.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"SwingUtil": "CLIENT_CLASS_000197"},
        )

    def test_r180_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r180.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 180):
