...
CLASS_COORDS = [
    ("CLIENT_CLASS_000036", "rs/I"),
    ("CLIENT_CLASS_000037", "rs/J"),
]
...
class Chat2SemanticReviewR41Tests(unittest.TestCase):
    def test_r41_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r41.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r41.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_448010991767A7F32856",
        )
        self.assertEqual(actual, expected)

    def test_r41_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r41.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "SceneTileModel": "CLIENT_CLASS_000036",
                "SceneTilePaint": "CLIENT_CLASS_000037",
            },
        )

    def test_r41_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r41.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 41):
