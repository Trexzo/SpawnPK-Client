...
CLASS_COORDS = [
    ("CLIENT_CLASS_000135", "rs/d/t"),
    ("CLIENT_CLASS_000142", "rs/d/z"),
]
...
class Chat2SemanticReviewR71Tests(unittest.TestCase):
    def test_r71_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r71.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r71.json")
        ...
        self.assertEqual(
            actual["review_id"],
            "SEMREVIEW_0DF83AB9E51DD7DACE67",
        )
        self.assertEqual(actual, expected)

    def test_r71_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r71.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "FloorOverlayDefinition": "CLIENT_CLASS_000135",
                "VarpDefinition": "CLIENT_CLASS_000142",
            },
        )

    def test_r71_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r71.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 71):
