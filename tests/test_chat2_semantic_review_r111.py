...
CLASS_COORDS = [
    ("CLIENT_CLASS_000426", "rs/l/e/f"),
    ("CLIENT_CLASS_000427", "rs/l/e/f$a"),
]
...
class Chat2SemanticReviewR111Tests(unittest.TestCase):
    def test_r111_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r111.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r111.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_2520A829D44C8379CCD5")
        self.assertEqual(actual, expected)

    def test_r111_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r111.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "ClientOverlay": "CLIENT_CLASS_000426",
                "OverlayLayer": "CLIENT_CLASS_000427",
            },
        )

    def test_r111_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r111.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 111):
