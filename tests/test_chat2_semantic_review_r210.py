...
class Chat2SemanticReviewR210Tests(unittest.TestCase):
    def test_r210_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r210.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r210.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_CDC6A4C7D7DF2EF2A443")
        self.assertEqual(actual, expected)

    def test_r210_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r210.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "BountyOverlay": "CLIENT_CLASS_000452",
            },
        )

    def test_r210_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r210.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 210):
