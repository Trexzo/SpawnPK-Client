...
CLASS_COORDS = [
    ("CLIENT_CLASS_000746", "rs/q/a/a/a/a"),
]
...
class Chat2SemanticReviewR151Tests(unittest.TestCase):
    def test_r151_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r151.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r151.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_77BBD1D38D47DCB43557")
        self.assertEqual(actual, expected)

    def test_r151_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r151.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "CustomMagicPresentationPacketHandler": "CLIENT_CLASS_000746",
            },
        )

    def test_r151_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r151.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 151):
