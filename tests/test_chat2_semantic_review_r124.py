...
CLASS_COORDS = [
    ("CLIENT_CLASS_000649", "rs/n/c/c/e"),
]
...
class Chat2SemanticReviewR124Tests(unittest.TestCase):
    def test_r124_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r124.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r124.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_2710F7C927590F200A4E")
        self.assertEqual(actual, expected)

    def test_r124_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r124.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "MailboxInterfacePacketHandler": "CLIENT_CLASS_000649",
            },
        )

    def test_r124_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r124.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 124):
