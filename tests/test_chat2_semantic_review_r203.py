...
CLASS_COORDS = [
    ("CLIENT_CLASS_000464", "rs/l/f/a/g/a"),
    ("CLIENT_CLASS_000466", "rs/l/f/a/g/c"),
]
...
class Chat2SemanticReviewR203Tests(unittest.TestCase):
    def test_r203_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r203.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r203.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_46A6B9E9F247B81FEF85")
        self.assertEqual(actual, expected)

    def test_r203_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r203.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "RaidPartyOverlay": "CLIENT_CLASS_000464",
                "RaidTheatreBar": "CLIENT_CLASS_000466",
            },
        )

    def test_r203_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r203.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 203):
