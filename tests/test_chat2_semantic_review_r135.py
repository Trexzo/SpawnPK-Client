...
CLASS_COORDS = [
    ("CLIENT_CLASS_000651", "rs/n/c/d/a"),
]
...
class Chat2SemanticReviewR135Tests(unittest.TestCase):
    def test_r135_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r135.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r135.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_2C4368F6B1F9C2755E43")
        self.assertEqual(actual, expected)

    def test_r135_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r135.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "RaidNavigationInterface": "CLIENT_CLASS_000651",
            },
        )

    def test_r135_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r135.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 135):
