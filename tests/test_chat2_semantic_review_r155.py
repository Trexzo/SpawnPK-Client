...
CLASS_COORDS = [("CLIENT_CLASS_000396", "rs/l/d/e")]
    ...
    for logical_id, internal_name in CLASS_COORDS:
        classes.append({
            ...
            "lineage": [{
                ...
                "relation": "BASELINE",
                "confidence": 1.0,
                "provenance": [{"authority": "EXACT_CURRENT_CLIENT", "source": "chat2-r155-fixture"}],
            }],
            "semantic_provenance": [],
        })
    return {
...
class Chat2SemanticReviewR155Tests(unittest.TestCase):
    def test_r155_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r155.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r155.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_5136D014B73E8CA7BE90")
        self.assertEqual(actual, expected)

    def test_r155_expected_stable_id(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r155.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {"LoginUpdateFetcher": "CLIENT_CLASS_000396"},
        )

    def test_r155_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r155.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}
        prior_names = set()
        prior_owners = set()
        for batch in range(2, 155):
