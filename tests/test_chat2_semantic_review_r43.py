...
class Chat2SemanticReviewR43Tests(unittest.TestCase):
    def test_r43_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r43.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r43.json")
    ...
    def test_r43_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r43.json")
        self.assertEqual(
    ...
    def test_r43_names_and_owners_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r43.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 43):
