...
CLASS_COORDS = [
    ("CLIENT_CLASS_000453", "rs/l/f/a/c/d"),
]
...
class Chat2SemanticReviewR201Tests(unittest.TestCase):
    def test_r201_resolves_deterministically(self):
        candidates = _load("mappings/candidates/v308.semantic.chat2.r201.json")
        expected = _load("mappings/candidates/v308.semantic-review.chat2.r201.json")
        ...
        self.assertEqual(actual["review_id"], "SEMREVIEW_997D257B6FB8A522CED6")
        self.assertEqual(actual, expected)

    def test_r201_expected_stable_ids(self):
        review = _load("mappings/candidates/v308.semantic-review.chat2.r201.json")
        self.assertEqual(
            {row["proposed_name"]: row["stable_id"] for row in review["proposals"]},
            {
                "CombatOverlay": "CLIENT_CLASS_000453",
            },
        )

    def test_r201_name_and_owner_do_not_overlap_prior_reviews(self):
        current = _load("mappings/candidates/v308.semantic-review.chat2.r201.json")
        names = {row["proposed_name"] for row in current["proposals"]}
        owners = {row["source_coordinate"]["owner"] for row in current["proposals"]}

        prior_names = set()
        prior_owners = set()
        for batch in range(2, 201):
