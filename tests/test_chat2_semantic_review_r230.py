...
CLASS_COORDS = [
    ("CLIENT_CLASS_001060", "rs/ui/components/b/a"),
    ("CLIENT_CLASS_001064", "rs/ui/components/b/e"),
]
    ...
    for logical_id, internal_name in CLASS_COORDS:
        classes.append({
            ...
            "lineage":[{
                ...
                "relation":"BASELINE",
                "confidence":1.0,
                "provenance":[{"authority":"EXACT_CURRENT_CLIENT","source":"chat2-r230-fixture"}],
            }],
            "semantic_provenance":[],
        })
    return {
...
class Chat2SemanticReviewR230Tests(unittest.TestCase):
    def test_r230_resolves_deterministically(self):
        actual=resolve_semantic_candidates(
            _class_lineage(),
            _member_lineage(),
            _load("mappings/candidates/v308.semantic.chat2.r230.json"),
        )
        expected=_load("mappings/candidates/v308.semantic-review.chat2.r230.json")
        self.assertEqual(actual["proposal_count"],2)
        self.assertEqual(actual["unresolved"],[])
        self.assertEqual(actual["review_id"],"SEMREVIEW_5789FBA171B6FB2F21B5")
        self.assertEqual(actual,expected)

    def test_r230_expected_stable_ids(self):
        review=_load("mappings/candidates/v308.semantic-review.chat2.r230.json")
        self.assertEqual(
            {row["proposed_name"]:row["stable_id"] for row in review["proposals"]},
            {
                "MaterialTab":"CLIENT_CLASS_001060",
                "MaterialTabGroup":"CLIENT_CLASS_001064",
            },
        )

    def test_r230_name_and_owner_do_not_overlap_prior_reviews(self):
        current=_load("mappings/candidates/v308.semantic-review.chat2.r230.json")
