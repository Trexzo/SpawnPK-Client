...
Gross relationship count before stable-symbol contradiction audit:

```text
matched        5,450 / 5,482
gross coverage 99.42%
unresolved        32
```

**Important later correction:** this is not precision-certified identity coverage.
A subsequent exact matched-method access-position audit found **23 definite cases** where
a same-symbol field relationship is contradicted by stronger bytecode behavior, plus five
single-observation leads. See:

`research/r2-chat2-stable-field-conflict-audit.md`

and:

`mappings/candidates/client5-to-v308.field-stable-conflicts.chat2.r2.json`

Those conflicting stable-symbol edges must be reconciled before a verified field-identity
...
The goal remains precision first. The 99.42% figure above is retained only as historical
gross matcher coverage; it is not a correctness claim after the contradiction audit.
