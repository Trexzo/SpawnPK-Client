...
## Post-R2C2 accepted semantic seed boundary

Main/Core explicitly accepted the full 39-proposal exact-v308 semantic seed at
`d8246f0bd2ded0d271a5401b1cf8a900fca7b008`:

- 32 class semantics;
- 3 field semantics;
- 4 method semantics.

Chat 2's R6G/R6H flow intelligence may now consume those accepted field/method semantics
through the canonical lineage objects supplied by Core. The acceptance itself remains
Main/Core authority; this research layer does not recreate, widen, or reinterpret that
acceptance.

In particular, accepted semantic names are usable as naming evidence for inferred source
parameters/locals, but the resulting `SRC_*` names remain separately inferred source-level
roles. They do not become original developer identifiers merely because their upstream
class/member semantic is accepted.
