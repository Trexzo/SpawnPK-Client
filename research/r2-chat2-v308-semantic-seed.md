...
`LOGIN_REWARD_IDX` parsing stores its integer directly into `rs/Client.P:I`.
...
## Current R2C2 boundary

The initial seed has since been expanded by exact-v308 interface/dossier research. The
current full Chat 2 candidate set resolves through R2C2 to:

- **86 proposals**
- **0 unresolved**
- review ID `SEMREVIEW_799C2D52A4528B4A66D0`
...
## Gameframe member expansion

A deeper exact-v308 audit of `rs/i/b` broadened the class role from an orb-only label to
`GameframeRenderer` and added six exact member candidates:

- `a()V` -> `renderGameframe` -> `CLIENT_METHOD_001933`
- `a(I)V` -> `selectChatTab` -> `CLIENT_METHOD_001934`
- `b()V` -> `renderChatButtons` -> `CLIENT_METHOD_001937`
- `c:Lrs/l/F;` -> `chatButtonSprite` -> `CLIENT_FIELD_002920`
- `d:Lrs/l/F;` -> `chatButtonHoverSprite` -> `CLIENT_FIELD_002921`
- `e:Lrs/l/F;` -> `chatButtonSelectedHoverSprite` -> `CLIENT_FIELD_002922`

The same class already has the Adventure orb sprite fields `f/g`. The broader class name
reflects that it owns both orb rendering and chat/gameframe presentation.
