# R2 Chat 2 — v308 class proposal risk preflight

The nine current Chat 2 class remap proposals were checked against the same exact-build
...
| Source | Semantic proposal | Exact class-name literal hits | Non-class resources under source package |
| --- | --- | ---: | ---: |
...
Combined R2A-style summary for these nine proposed source classes:
...
If the integration lane later promotes these proposals into an actual R2A `remap_spec`,
the current R2B fail-closed checks would not require:

- `rewrite_class_name_strings`
- `allow_package_resource_risk`

for these mappings based on the present exact-v308 risk categories.

This is a preflight finding only. Chat 2 does not authorize or execute the remap.
